...
from bof_model import BoFModel
import dataloader
import trainval
...
def main():
    ...
    model = BoFModel(dataloaders_dict, inplanes=3, feature_dim=latent_dim, num_classes=total_classes)
    print (model)
    ...
    print (out_dir)
    if feature_extract:
        torch.save(final_model, os.path.join(out_dir, 'sift_' +str(latent_dim) + '_final_model_ft'))
        final_stats_file = open (os.path.join(out_dir, 'sift_' +str(latent_dim)+ '_finalstats_ft.txt'), 'w')
    else:
        torch.save(final_model, os.path.join(out_dir, 'sift_' +str(latent_dim)+ '_final_model'))
        final_stats_file = open (os.path.join(out_dir,  'sift_' +str(latent_dim)+ '_finalstats.txt'), 'w')
    csv_file = open(os.path.join(out_dir,  'sift_' +str(latent_dim)+ '_results.csv'), 'w')
